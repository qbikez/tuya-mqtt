...
const main = async () => {
  ...
  try {
    ...
    console.error(
      `Could not parse new devices config file [${configNewDevicesFilename}]!`
    );
    console.error(e);
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(
      `Could not parse devices config file [${configDevicesFilename}]!`
    );
    console.error(e);
    process.exit(1);
  }
  ...
  try {
    console.log(
      `Backing up devices config file [${configDevicesFilename}] to [${configDevicesBackupFilename}]...`
    );
    fs.copyFileSync(
      `./${configDevicesFilename}`,
      `./${configDevicesBackupFilename}`
    );
  } catch (e) {
    console.error(
      `Could not make backup of devices config file [${configDevicesFilename}] to [${configDevicesBackupFilename}].`
    );
    console.error(e);
    process.exit(1);
  }
  ...
  for (const configNewDevice of configNewDevices) {
    const configDevice = configDevicesDictionary[configNewDevice.id];
    if (configDevice == null) {
      // Add new device
      console.log(
        `Adding device: ${configNewDevice.name} (id: ${configNewDevice.id})...`
      );
    ...
    if (configDevice.allowMerge === false) {
      continue;
    } // No merge updates allowed for this device

    // Update existing device
    if (configDevice.name !== configNewDevice.name) {
      console.log(
        `Updating device name: '${configDevice.name}' to '${configNewDevice.name}' (device id: ${configDevice.key})...`
      );
      configDevice.name = configNewDevice.name;
    }
    if (configDevice.key !== configNewDevice.key) {
      console.log(
        `Updating device key for: '${configDevice.name}' (device id: ${configDevice.key})...`
      );
  ...
  try {
    ...
    fs.writeFileSync(`./${configDevicesFilename}`, configDevicesJson, {
      encoding: "utf8",
    });
  } catch (e) {
    console.error(
      `Could not write devices config file [${configDevicesFilename}]!`
    );
    console.error(e);
    process.exit(1);
  }
